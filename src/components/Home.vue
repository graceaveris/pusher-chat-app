<template>
    ...
    <div class="link-bar">
      <p>Invite to chat by sharing this link</p>
          ...
          channel.bind('pusher:subscription_succeeded', members => {
              ...
              members.each((member) => {
                ...
                if (member.id === channel.members.me.id) {
                  ...
                  this.activeUser.userId = member.id
                  this.activeUser.userName = 'Anonymous-' + member.id.substr(2, 5)
                } else {
                  //all other users
                  this.users.push(this.createMember(member.id))
                }
          ...
          channel.bind('client-send-username', (res) => { //{data: this.activeUser})
              this.users.forEach((user, index) => {
                if (user.id === res.userId) {
                   this.users[index].name = res.userName
                }
            })
          })
